...
function _any_install_main {
  ...
  function main {
    ...
    if (Test-Path env:ANY_INSTALL_EXAMPLE_INSTALL) { $install_dir = (Get-Item env:ANY_INSTALL_EXAMPLE_INSTALL).Value } else { $install_dir = "${HOME}/.any-install-example" }
    ...
    anyi_install_executable "${asset}" "${install_dir}/any-install-example.ps1"
